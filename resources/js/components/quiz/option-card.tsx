...
export default function OptionCard({
    ...
    // Wrong answers shake; correct answers pop — gated behind reduced-motion.
    const animate =
        !reduce && answered && selected && status === 'wrong'
            ? { x: [0, -9, 9, -7, 7, 0] }
            : !reduce && status === 'correct'
              ? { scale: [1, 1.035, 1] }
              : {};
    ...
    return (
        ...
        >
            <span
                className={`flex h-[clamp(1.7rem,min(6vw,5vh),2.5rem)] w-[clamp(1.7rem,min(6vw,5vh),2.5rem)] flex-none items-center justify-center rounded-full font-display text-[clamp(0.8rem,min(2.6vw,2.8vh),1.05rem)] font-bold transition-colors ${badge[status]}`}
            >
                {status === 'correct' && answered ? (
